...
export default function Block_1() {
    ...
    // Randomizer for RAIN COMPONENTS from images under wampus
    ...
    // Randomizer for RAIN CLASSES from images under wampus
    const getRandomBadgeClass = () => {
        // Get animation classes for images
                ...
                {isSmallScreen || isPhone ?
                    <>
                        <h1 className={`${resStyles('title', resolutions)}`}>
                            Storm Shop: <a href='/' style={{ color: '#FFD645', textDecoration: 'none' }}>Scripts</a> <span style={{ color: '#96D9FF', textDecoration: 'none' }}>&</span> <a href='/' style={{ color: '#FF7BB3', textDecoration: 'none' }}>Subscriptions</a>
                        </h1>
                        <p className={`${styles.content} ${resStyles('subtitle', resolutions)}`}>
                            Buy subscriptions in 2 clicks! Creating your own project?
                            Buy ready-made scripts in our store! Not found? It happens...
                            then order your own script, bot or even website!
                            <br />
                            <a href='/' style={{ color: '#FF7BB3', fontWeight: '500' }}>Subscriptions</a>, <a href='/' style={{ color: '#FFD645', fontWeight: '500' }}>Script Store</a>, <a href='/' style={{ color: '#BAEAFF', fontWeight: '500' }}>Order for yourself</a>
                        </p>
                    </>
                    : <></>
                }
                ...
                <div className={`${styles.text} ${resStyles('main_text', resolutions)}`}>
                    ...
                    {!isSmallScreen && !isPhone ?
                        <>
                            ...
                            {/* Main content */}
                            <p className={`${styles.content} ${resStyles('subtitle', resolutions)}`}>
                                Purchase subscriptions effortlessly in just two clicks!
                                <br /><br />
                                <b>Starting a new project?</b>
                                <br />
                                Explore our store for ready-made scripts tailored to your needs.
                                <br /><br />
                                <b>Can't find what you're looking for?</b>
                                <br />No problem — simply place a custom order for a personalized script, bot, or even a complete website!
                                <br />
                                {/* <a href='/' style={{ color: '#FF7BB3', fontWeight: '500' }}>Subscriptions</a>, <a href='/' style={{ color: '#FFD645', fontWeight: '500' }}>Script Store</a>, <a href='/' style={{ color: '#BAEAFF', fontWeight: '500' }}>Order for yourself</a> */}
                            </p>
                            {/* <p className={`${styles.content} ${resStyles('subtitle', resolutions)}`}>
                                You can also earn together with us! Upload
                                your scripts and we will sell them for you!
                                Don't know how to program?
                            </p> */}
                            {/* <p className={`${styles.content} ${resStyles('subtitle', resolutions)}`}>
                                Then an incredible referral program. 100%
                                profit from the store!
                                <br /><a>Learn more</a>
                            </p> */}
                            {/* Buttons */}
