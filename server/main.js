    ...
    while (i < required_params.length) {
        ...
        if (type != param[1]) {
            ...
            if (!approved) {
                // 400 Bad Request
                res.json({
                    error: `${param[0]} is invalid! Expected to receive "${param[1]}", but received type "${type}"!`,
                    success: false
                }).status(400);
...
app.post(`${versioning.prefix}/signup`, async (req, res) => {
    ...
    if (!(await userValidation.verifyUserCaptcha(token))) {
        res.json({
            success: false,
            error: "You did not pass the captcha. Please try again!"
        }).status(406);
    ...
    if (existing_user) {
        res.json({
            error: `User "${req.body.username}" has already been registered!`,
            success: false
        }).status(409)
        return;
    }
    ...
    if(userByUuid !== undefined) {
        res.json({
            error: `You already have an account, log into your existing account!`,
            success: false
        }).status(409)
        return;
    }
